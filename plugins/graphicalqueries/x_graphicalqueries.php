...
// Add
if ($_REQUEST['selectMethode'] == 'add') {
	$queryResult = SelectTools::add($jsonPMResult, $queryResult);

// Remove
} else if ($_REQUEST['selectMethode'] == 'del') {
	$queryResult = SelectTools::del($jsonPMResult, $queryResult);

// Intersect
} else if ($_REQUEST['selectMethode'] == 'intersec') {
	$queryResult = SelectTools::intersect($jsonPMResult, $queryResult);
}
...
if (isset($queryResult)) {
	...
	echo "{\"mode\":\"$mode\", \"queryResult\":0, \"needRefresh\":1}";
}
?>